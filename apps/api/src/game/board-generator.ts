...
import { getCatanHexPositions, getNeighbors, axialToCube } from '../../../../libs/shared/src';
...
export function generateBoard(maxRetries = 100): BoardState {
    ...
    console.warn('Board generation retry limit exhausted. Returning last attempt.');
    ...
    const shuffledTerrains = shuffle(TERRAINS);
    const hexes: Hex[] = positions.map((pos, index) => ({ q: pos.q, r: pos.r, terrain: shuffledTerrains[index], number: null }));
    const shuffledNumbers = shuffle(NUMBERS);
    hexes.filter(h => h.terrain !== 'desert').forEach((h, i) => h.number = shuffledNumbers[i]);
    const ports = generatePorts(hexes);
    return { hexes, ports };
}
