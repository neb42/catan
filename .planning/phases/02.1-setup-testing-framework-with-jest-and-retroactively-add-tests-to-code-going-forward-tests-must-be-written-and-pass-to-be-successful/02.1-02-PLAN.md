...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/api/src/websocket/connection-manager.spec.ts"
      to: "ws"
      via: "mocks WebSocket with jest.fn()"
      pattern: "jest.fn.*send"
---

<objective>
Add comprehensive test coverage for all API WebSocket infrastructure using Jest, including schema validation, manager classes, and integration tests.

Purpose: Ensure existing WebSocket code is tested before building additional features
Output: Full Jest test suite for API covering schemas, managers, and integration scenarios
</objective>
...
Use Jest syntax (describe, it, expect) to test schemas following RESEARCH.md Pattern 4 "Testing Zod Schemas":
...
Why this approach: Zod schemas are the contract between client and server. Testing validates both valid inputs (expected to parse) and invalid inputs (expected to fail with specific errors). Use safeParse() in tests to inspect validation failures without throwing. Jest provides excellent assertion messages and is the standard for Node.js testing.
  </action>
  <verify>
```bash
nx test api --testNamePattern="schema"
```

All schema tests pass, invalid inputs are properly rejected.
  </verify>
  <done>
Schema validation tests cover all message types with valid and invalid cases using Jest.
  </done>
</task>
...
Use Jest syntax and mocking to test manager class following RESEARCH.md Pattern 3 "Unit Testing Managers with Mock Dependencies":
...
Test heartbeat with Jest fake timers (adapted from RESEARCH.md Pattern 6):
- Use jest.useFakeTimers() in beforeEach
- Advance time by 30 seconds with jest.advanceTimersByTime(), verify ping called
- Simulate no pong response, advance another 30s, verify terminate called
- Use jest.useRealTimers() in afterEach

Mock WebSocket using Jest:
```typescript
...
Mock dependencies (ConnectionManager, RoomManager) using jest.fn().

Why this approach: Unit tests isolate business logic from WebSocket infrastructure. Mock WebSocket objects with jest.fn() to verify manager behavior without real network connections. Jest fake timers make heartbeat tests deterministic and instant. Jest's mocking is more mature and integrated than Vitest's for Node.js testing.
  </action>
  <verify>
```bash
nx test api --testNamePattern="Manager|Router"
```

All manager and router unit tests pass.
  </verify>
  <done>
Manager classes have comprehensive unit test coverage with Jest-mocked dependencies.
  </done>
</task>
...
Use Jest with real WebSocket connections following RESEARCH.md Pattern 2 "WebSocket Integration Testing":
...
Why this approach: Integration tests verify the full WebSocket stack works end-to-end. Using real WebSocket connections (not mocks) catches issues like message serialization, connection handling, and race conditions. TestWebSocket helper prevents flaky tests by providing timeout-protected promises. Jest provides excellent async test support with built-in timeout handling.
  </action>
  <verify>
```bash
nx test api --testNamePattern="integration"
```

All integration tests pass, WebSocket server handles real connections correctly.
  </verify>
  <done>
Integration test suite verifies complete WebSocket server functionality with real connections using Jest.
  </done>
</task>
...
3. Manager unit tests use Jest-mocked dependencies
4. Integration tests use real WebSocket connections
5. Coverage thresholds met (80%+ lines/functions)
6. `nx test api` completes successfully with Jest
</success_criteria>
