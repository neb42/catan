...
Fix:
1. First, verify the DIRS array and determine correct mapping:
   - Read actual DIRS array from board-generator.ts
   - For each DIRS[i], identify which angle-ordered edge it represents
   - Confirm mapping: DIRS index → angle edge

2. In generatePorts function, after finding bestEdge from DIRS array (line ~228), map the CUBE_DIRECTIONS index to angle-ordered index:

```typescript
// Map CUBE_DIRECTIONS index to angle-ordered edge index
// Verified mapping (see debug report for derivation):
// DIRS[0] → angle edge 1 (SE/60°)
// DIRS[1] → angle edge 0 (E/0°)
// DIRS[2] → angle edge 5 (NE/300°)
// DIRS[3] → angle edge 4 (NW/240°)  
// DIRS[4] → angle edge 3 (W/180°)
// DIRS[5] → angle edge 2 (SW/120°)
const DIRS_TO_ANGLE_MAP = [1, 0, 5, 4, 3, 2]; // Maps DIRS index to angle edge
const angleOrderedEdge = DIRS_TO_ANGLE_MAP[bestEdge];
```

3. Use angleOrderedEdge when creating the port:
...
7. Specific test: For any port previously misplaced (inside board), verify it now appears at correct coast edge
8. Check browser console for getPortPosition calculation logs if available
  </verify>
  <done>
