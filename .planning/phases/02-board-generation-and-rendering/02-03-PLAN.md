...
<task type="auto">
  ...
  <files>apps/api/src/handlers/websocket.ts, apps/web/src/components/Lobby.tsx</files>
  <action>
...
2. Find the "all players ready" handler (likely in toggle_ready or similar message handler)

3. When countdown finishes and game should start:
```typescript
// Generate board
...
**On client (Lobby.tsx or new routing logic):**
...
2. Add state for game started:
```typescript
const [gameStarted, setGameStarted] = useState(false);
```

3. In handleMessage callback, add case for game_started:
```typescript
case 'game_started': {
...
4. Update render logic to show Game component when started:
```typescript
if (gameStarted) {
  return <Game />;
}
