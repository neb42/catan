...
files_modified:
  ...
  - apps/api/src/game/largest-army-logic.spec.ts
...
must_haves:
  ...
  artifacts:
    ...
    - path: 'apps/api/src/game/largest-army-logic.spec.ts'
...
Run tests to confirm green: `npx vitest run apps/api/src/game/largest-army-logic.spec.ts`
</action>
<verify>
Run `cd apps/api && npx vitest run src/game/largest-army-logic.spec.ts` - all tests pass
</verify>
<done>
...
<verification>
1. Run `cd apps/api && npx vitest run src/game/largest-army-logic.spec.ts` - all TDD tests pass
