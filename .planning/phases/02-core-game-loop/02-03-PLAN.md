...
Wire up WebSocket sync by adding message handler in websocket service or useWebSocket hook:
- When 'game_state' message received, call `useGameStore.getState().updateGameState(message.gameState)`

Store is lightweight - just game state and UI selections. No complex logic here.
  </action>
  <verify>
