...
1. Create room
2. Send 'start_game' message → Receive 'game_state' with initial board and placementRound=1
3. Send 'place_settlement' for currentPlayer → Receive updated 'game_state' with settlement placed
4. Send 'place_road' for same player → Receive 'game_state' with road placed and next player's turn
5. Test error cases: Send placement from non-current player → Receive 'error' message
6. Complete 8 rounds → Verify phase transitions to 'gameplay'

Expected outcomes:
- Valid placements: gameState updates, settlements/roads arrays grow, currentPlayer advances
- Invalid placements: error message returned, gameState unchanged
- Phase transition: After round 8, phase='gameplay', turnPhase='roll'
  </verify>
  <done>
