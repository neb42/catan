...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/api/src/game/BoardGenerator.ts"
      ...
      min_lines: 100
    - path: "apps/web/src/game/utils/coordinates.ts"
  ...
  key_links:
    ...
    - from: "apps/api/src/game/BoardGenerator.ts"
      to: "apps/web/src/game/utils/coordinates.ts"
      via: "uses hex coordinate functions"
      pattern: "(getNeighbors|axialToCube)"
---
...
<task type="auto">
  <name>Task 3: Create hex coordinate utility functions</name>
  <files>apps/web/src/game/utils/coordinates.ts</files>
  <action>
Create `apps/web/src/game/utils/coordinates.ts` with cubic coordinate system utilities following Red Blob Games patterns from RESEARCH.md Pattern 2:
...
This is server-agnostic math, placed in web for now but can be moved to shared if server needs it later.
  </action>
  <verify>
Create simple test: verify getNeighbors({q:0, r:0}) returns 6 distinct coordinates. Verify cubeDistance({q:0,r:0,s:0}, {q:2,r:0,s:-2}) === 2.
  ...
  <done>
Hex coordinate utilities exist with all 5 functions implemented. Manual verification confirms neighbor finding and distance calculations work correctly.
  </done>
</task>
...
Import coordinates.ts functions (copy from web or reference honeycomb-grid). Use mode from env var `BOARD_GEN_MODE` (default 'balanced').
