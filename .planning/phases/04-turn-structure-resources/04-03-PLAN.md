...
<task type="auto">
  ...
  <action>
    ...
    3. Add effect to show notification when player receives resources:
       - Watch for lastDiceRoll changes
       - After dice animation completes (use onAnimationComplete callback)
       - Check if current player received resources in resourcesDistributed
    ...
    4. Store resourcesDistributed in component state or gameStore:
       - Add lastResourcesDistributed to gameStore TurnSlice
       - Or pass as prop from parent component
       - Decision: Store in gameStore for easier access

    5. Update dice_rolled handler in Lobby.tsx to also store resourcesDistributed:
       ```typescript
       gameStore.setLastResourcesDistributed(message.resourcesDistributed);
       ```

    6. Add to gameStore:
       - lastResourcesDistributed: Array<{ playerId: string, resources: Array<{ type: string, count: number }> }> | null
       - setLastResourcesDistributed action
       - useLastResourcesDistributed hook
  ...
  <done>
    ...
    - lastResourcesDistributed stored in gameStore
  </done>
</task>
