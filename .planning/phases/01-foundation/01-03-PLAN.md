...
- Import useState for local state (roomState, currentView)
...
- State management: Track current room (Room from @catan/shared or null), current view ('create' | 'join' | 'lobby')
- Render conditionally:
  - If no room: Show CreateRoom and JoinRoom components side-by-side
  - If in room: Show room ID (with copy button), PlayerList, connection status indicator
