...
**Test Case 8: Color Selection (LOBBY-04)**
1. In Window 1 (Alice), click color selector, choose "blue"
2. Verify: Alice's color indicator changes to blue in all windows
3. In Window 2 (Bob), try to select "blue" (already taken by Alice)
4. Verify: Error message "Color already taken" or blue option disabled
5. In Window 2 (Bob), select "red"
6. Verify: Bob's color indicator shows red in all windows

**Test Case 9: Ready Status (LOBBY-05)**
1. In Window 1 (Alice), click "Ready" button
2. Verify: Alice shows ready status (✓) in all windows
3. Verify: Button changes to "Not Ready" or similar
4. Click again to toggle off
5. Verify: Alice shows not ready (✗) in all windows

**Test Case 10: Game Start Countdown (LOBBY-06)**
1. Ensure 3 players in room (Alice, Bob, Carol)
2. Each player clicks "Ready" button
3. Verify: When 3rd player becomes ready, countdown appears showing "Game starting in 5 seconds..."
4. Verify: Countdown decrements every second: 5, 4, 3, 2, 1
5. Verify: All windows show synchronized countdown (< 500ms difference)
6. Test with 4 players - verify countdown also works

**Test Case 11: Player Leave**
1. In Window 2 (Bob), close tab or browser
2. Verify: Other windows show Bob removed from player list within a few seconds

**Test Case 12: Grace Period Cleanup**
...
**Test Case 13: WebSocket Reconnection**
...
- ✅ LOBBY-05: User can toggle ready status
- ✅ LOBBY-06: Countdown starts when all players ready (3-4 players)
...
- [ ] Color selection works with conflict detection
- [ ] Ready status toggles correctly for all players
- [ ] Game start countdown appears when all ready
</success_criteria>
