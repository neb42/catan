...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/web/src/routes/index.tsx"
      to: "useWebSocket().sendMessage"
      via: "Send JOIN_ROOM message after NICKNAME_ACCEPTED before navigation"
      pattern: "sendMessage.*JOIN_ROOM"
    - from: "apps/web/src/routes/index.tsx"
      to: "navigate({ to: '/lobby' })"
      via: "Navigate to lobby after JOIN_ROOM sent"
...
- Trim nickname
- Check length >= 3 characters → error: "Please enter a nickname"
- Check length <= 30 characters → error: "Nickname too long (max 30 characters)"
- Check status === 'connected' → error: "Connection lost. Please wait..."

WebSocket message handling:
- On NICKNAME_ACCEPTED:
  1. Send JOIN_ROOM message to actually join the lobby room:
     ```
     sendMessage({
       type: 'JOIN_ROOM',
     ...
     ```
  2. setTimeout 300ms then navigate({ to: '/lobby' }) (transition delay)
- On NICKNAME_REJECTED: setError(message.payload.message), setIsSubmitting(false)

**CRITICAL FLOW:** Client must send JOIN_ROOM after receiving NICKNAME_ACCEPTED and before navigating. This ensures:
- Client's nickname is stored in room's nickname map (done by SET_NICKNAME handler)
- Client's ID is added to room's clients set (done by JOIN_ROOM handler)
- No data inconsistency between nickname storage and room membership

Why this sequence:
- SET_NICKNAME validates and stores nickname, but does NOT add client to room
- JOIN_ROOM adds client to room's clients set
- Without JOIN_ROOM, client has nickname but isn't in room.clients, breaking Phase 1 room system design
- This wires nickname validation to room membership per Phase 1 architecture
...
- Helper text: "3-4 players • Choose colors • Play together"
...
- WebSocket connection status visible when not connected
  ...
  <done>
Landing page route created with controlled form, client validation, WebSocket message handling (including JOIN_ROOM send after NICKNAME_ACCEPTED), and navigation logic. TypeScript compilation succeeds. Page renders and form accepts input.
  </done>
</task>
...
@keyframes:
- slide-up: translateY(40px) + opacity 0 → 0 + 1
- shake: translateX oscillation (-8px, 8px, 0)
- slide-in-left: translateX(-20px) + opacity 0 → 0 + 1
...
Complete user flow:
1. User loads app → sees landing page with animated background
2. User types nickname → character counter updates (0-30)
3. User submits empty/short nickname → sees client-side error with shake animation
4. User submits valid nickname → button shows "Joining..." loading state
5. Server validates uniqueness → responds with NICKNAME_ACCEPTED or NICKNAME_REJECTED
6. If accepted → client sends JOIN_ROOM message → page transitions to lobby placeholder
7. If rejected (duplicate) → error message appears with slide-in animation
  </what-built>
  <how-to-verify>
...
4. Test client-side validation:
   - [ ] Type "ab" (2 chars) → submit → error "Please enter a nickname"
   - [ ] Input shakes on error
   - [ ] Error message slides in below input
   - [ ] Type "c" to make "abc" → error clears immediately
   - [ ] Character counter updates to "3/30"

5. Test successful submission:
   - [ ] Enter valid nickname (e.g., "Alice") → submit
...
6. Test duplicate nickname rejection:
   ...
   - [ ] Enter same nickname ("Alice") → submit
   - [ ] Error message appears: "This nickname is already taken. Try another!"
   - [ ] Button re-enables for retry
   - [ ] Enter different nickname ("Bob") → submit → navigates to lobby

7. Test connection status:
   - [ ] Stop API server (Ctrl+C in Terminal 1)
   - [ ] Refresh page → connection status shows "Reconnecting..."
   - [ ] Submit button disabled when not connected
   - [ ] Restart API server → status changes to "Connected" (or disappears)
   - [ ] Submit button re-enables

8. Test emoji and special characters:
   - [ ] Enter nickname with emoji "Alice 😊" → character count accurate
   - [ ] Submit → accepted if unique
   - [ ] Enter 30-character nickname → counter shows "30/30"
   - [ ] Try to type 31st character → blocked by maxLength
...
6. Client sends JOIN_ROOM message after NICKNAME_ACCEPTED before navigation
7. Navigation to /lobby occurs after JOIN_ROOM sent
8. Error messages display on rejection with animations
9. Lobby placeholder route exists and renders
10. All CSS animations (slide-up, shake, slide-in, pulse) work
...
- JOIN_ROOM message sent before navigation to wire nickname validation to room membership
</success_criteria>
