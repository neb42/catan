...
- Winner card: 3px gold border (#f1c40f), slightly larger, gold background tint, crown/trophy emoji (🏆) next to name
- Non-winner cards: 1px neutral border (#d7ccc8), white background
- Header: Avatar using `PLAYER_COLOR_HEX[player.color]` for background + Nickname + Winner badge (if winner) + Total VP badge
- Body: Group of small badges/text showing VP breakdown:
  - Settlements: {count} (×1 VP)
  - Cities: {count} (×2 VP)
  - Longest Road: 2 VP (only if > 0)
  - Largest Army: 2 VP (only if > 0)
  - VP Cards: {count} VP (only if > 0)

Per CONTEXT.md locked decision: "Use player colors from the game for their data in charts/stats (not parchment neutrals)" - apply player.color via PLAYER_COLOR_HEX to avatar background.
...
<done>
VictoryModal displays ResultsBreakdown and StatisticsTabs. Content order: winner → results → statistics → buttons. Modal has scroll capability.
</done>
</task>
...
1. **Store integration complete** — gameStats field in store, set by victory WebSocket handler
2. **ResultsBreakdown displays rankings** — Card-based layout, detailed VP breakdown, winner distinction clear
3. **StatisticsTabs integrated** — Renders below results in VictoryModal, shows all three tabs
4. **Modal scrollable** — Can view all content even if statistics are extensive
   </success_criteria>
