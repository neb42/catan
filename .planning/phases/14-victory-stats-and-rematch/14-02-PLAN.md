...
must_haves:
  truths:
    - 'Bar charts render correctly without errors'
    - 'Dice frequency distribution displays as bar chart visualization'
    - 'Dev card stats show per-player breakdown with player colors'
    - 'Resource stats show distribution, net flow, and trade activity separately'
  artifacts:
...
Per CONTEXT.md locked decision: "Frequency distribution (how many times each number 2-12 was rolled) with bar chart visualization"

Transform data into array format: `[{ number: 2, count: 5 }, { number: 3, count: 8 }, ...]` for numbers 2-12.

Use Mantine BarChart showing frequency distribution - the bar height represents how many times each number was rolled:
...
The bar chart clearly visualizes frequency distribution - taller bars = rolled more often, shorter bars = rolled less often. Each bar represents the count for that dice number.
...
Per CONTEXT.md locked decision: "Resources collected from dice rolls (distribution), Resources gained vs resources spent (net flow), Trade activity (resources traded)"

Show three subsections implementing the exact locked requirements:

1. **"Resources Collected (Distribution)"** - Show distribution breakdown per resource type per player. Stacked bar chart with players on X-axis, bars stacked by resource type (wood, brick, sheep, wheat, ore). This shows WHO collected WHAT resources from dice rolls.

2. **"Net Resource Flow (Gained vs Spent)"** - Calculate and display net flow: `gained - spent` for each resource type per player. Use grouped bar chart showing positive (gained more) and negative (spent more) values. Include visual comparison with bars above/below zero line.

3. **"Trade Activity"** - Show net trade flow per player: positive = received more than gave, negative = gave more than received. Display total resources traded (net) per player.

Use Mantine BarChart with `type="stacked"` for distribution section. Use player colors from PLAYER_COLOR_HEX for player identification in all sections. Include legend mapping players to their game colors.
