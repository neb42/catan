...
must_haves:
  truths:
    ...
    - 'User can see building costs in tooltips on buttons'
...
- Each button shows icon + label + remaining count
- Tooltip on hover shows resource cost (icons only)
...
// Resource icons (use SVG or emoji for now - can polish later)
const ResourceIcon = ({ type }: { type: string }) => { ... };

const CostTooltip = ({ cost }: { cost: Record<string, number> }) => (
  <Group gap={4}>
    {Object.entries(cost).map(([resource, amount]) => (
      <Group key={resource} gap={2}>
...
export function BuildControls() {
  ...
  return (
    <Group gap="sm">
      <Tooltip label={roadCheck.disabledReason || <CostTooltip cost={BUILDING_COSTS.road} />}>
        <Button
          onClick={() => setBuildMode('road')}
          disabled={!roadCheck.canBuild}
        >
          Road ({remaining.roads})
        </Button>
      </Tooltip>

      <Tooltip label={settlementCheck.disabledReason || <CostTooltip cost={BUILDING_COSTS.settlement} />}>
        <Button
          onClick={() => setBuildMode('settlement')}
          disabled={!settlementCheck.canBuild}
        >
          Settlement ({remaining.settlements})
        </Button>
      </Tooltip>

      <Tooltip label={cityCheck.disabledReason || <CostTooltip cost={BUILDING_COSTS.city} />}>
        <Button
...
Buttons show remaining piece count.
Tooltips show cost (when enabled) or disabled reason.
Cancel button appears when in build mode.
</done>
</task>
...
- BuildControls shows Road/Settlement/City buttons with piece counts
- Disabled buttons have clear tooltip explaining why disabled
- Enabled buttons have tooltip showing resource cost icons
  </success_criteria>
