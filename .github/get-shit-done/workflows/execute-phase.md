...
1. **Spawn agent for checkpoint plan:**
   ```
   #tool:runSubagent:general-purpose <subagent-task-prompt>
   ```
...
6. **Spawn continuation agent (NOT resume):**
   ...
   ```
   #tool:runSubagent:general-purpose <filled_continuation_template>
   ```
...
```
#tool:runSubagent:gsd-verifier Verify phase {phase_number} goal achievement.
